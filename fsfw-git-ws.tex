...
%       text height=4ex,
    ...
    \node[location] (untracked) {untracked / \\ unstaged \\ ("`working tree"')};
    \node[location] (staged) [right of=untracked] {staged \\("`index"') \\~};
    \node[location] (committed) [right of=staged] {local \\ repository \\~};
    \node[location] (published) [right of=committed] {remote \\ repository \\~};

    \draw[->, thick, color=black!80!white] (untracked.east) -- (staged.west)
          node [midway, above=4.5ex, color=black] {\texttt{git add}};
    \draw[->, thick, color=black!80!white] (staged.east) -- (committed.west)
          node [midway, above=4.5ex, color=black] {\texttt{git commit}};
    \draw[->, thick, color=black!30!white] (committed.east) -- (published.west)
          node [midway, above=4.5ex, color=black] {\texttt{git push}};
  \end{tikzpicture}
\end{center}
...
\frametitle{Noch zu erwähnen (ggf. mit eigener Folie)}
\begin{itemize}
\item diff / difftool
\item branches, branching model
\item merge (Konflikte) rebase
\item .gitignore
\item github $\neq$ git
