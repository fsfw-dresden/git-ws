...
\item<2-> Ein Branch (default: \textit{master}) ist immer aktiv ("`HEAD")
\item<3-> beliebig viele Branches möglich
\item<4-> Branch wechseln: \texttt{git checkout \only<4,6>{bugfix} \only<5>{master}}
\item<7-> Neuen Branch erstellen: \texttt{git checkout -b experiment}
\end{itemize}
\vspace{-3mm}
