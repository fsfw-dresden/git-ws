...
\begin{tikzpicture}
      ...
      \gitDAG[grow down sep = 1em]{
        ...
        \gitbranch
        {master}     % node name and text
        {above=of bugfix} % node placement
        {E}
...
\begin{frame}
  ...
  \item \texttt{git add <conflicting-file>}
...
\begin{enumerate}
 \setcounter{enumi}{\value{taskcounter}}
  ...
  \begin{enumerate}[a)]
   ...
   \item ignore-Datei anlegen: \verb|echo "__pycache__" >> .gitignore|
   \item \verb|.gitignore| comitten: \\ \verb|git add .gitignore; git commit -m "add gitignore"|
...
\begin{itemize}
\item Jede Zeile in \verb|.gitingore| ist ein Muster für einen Dateinamen
\item Unterstützt wildcard $*$
\item sonst ganze Pfadkomponenten
\end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item Weitere Tipps:
\begin{itemize}
\item Informative Bash-Erweiterung (Bild), Link
\item Aliase in \verb|.gitconfig| (z.B.: \verb|git co| $\rightarrow$ \verb| git checkout|)
\item Globale gitignore-Datei: Link
\item Bewährtes Branching-Modell anwenden: Link
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[label=uj]{Schlussbemerkungen (2)}
\begin{itemize}
 \item Fragen?\\[10mm]
...
\begin{frame}[label=link10]{Quellen und Links}
Weitere Ressourcen:
\tiny
\begin{itemize}
 \item
